...
class AppWaveLink {
    ...
    init() {
        ...
        this.keyIconsHardware = {
            'default': new SVGIconWL({ icons: `./images/key/hardwareActions/default.svg`, icon: `default`}),
            'setOutputDevice': new SVGIconWL({ icons: `./images/key/hardwareActions/setOutputDevice.svg`, icon: `setOutput`}),
            'setOutputDeviceActive': new SVGIconWL({ icons: `./images/key/hardwareActions/setOutputDeviceActive.svg`, icon: `setOutputActive`}),
            'toggleOutputDeviceFirst': new SVGIconWL({ icons: `./images/key/hardwareActions/toggleOutputDeviceFirst.svg`, icon: `decrease`}),
            'toggleOutputDeviceSecond': new SVGIconWL({ icons: `./images/key/hardwareActions/toggleOutputDeviceSecond.svg`, icon: `default`})
        };
        ...
        this.secondaryIconType = [ 'Monitor', 'MonitorMute', 'Stream', 'StreamMute', 'All', 'AllMute', 'Set', 'Add', 'Active' ];
        ...
        primaryIconTypeInput.forEach ( primType => {
            this.secondaryIconType.forEach(secType => {
                ...
                switch (primType) {
                    case 'wave':
                        if (secType == 'Add' || secType == 'Active')
                            return;
                    default:
                        break;
                }

                if (secType != 'Set' && secType != 'Add' && secType != 'Active' && !secType.includes('All')) {
                    if (primType == 'wave') {
        ...
        primaryIconTypeHardware.forEach ( icon => {
            ...
            this.keyIconsHardware[`${icon[1]}`] = new SVGIconWL({ icons: `./images/key/hardwareActions/hardwareSettings/${icon[0]}.svg`, icon: `${icon[1]}` });
        });
