...
/// <reference path="simple-jsonrpc-js.min.js" />
        ...
        this.call(kJSONPropertyGetApplicationInfo).then((result) => {
            if (result || result == undefined) {
                ...
                if (result[kJSONKeyInterfaceRevision] >= kJSONPropertyMinimumSupportedAPIVersion) {
                    ...
                    debug(`Supported api version for ${kJSONPropertyAppName} found: ${this.apiVersion}`);
                    this.getMicrophoneConfig();
                    this.getSwitchState();
                    ...
                    if (result[kJSONKeyInterfaceRevision] > kJSONPropertyMaximumSupportedAPIVersion)
                        debug(`Wrong api version for ${kJSONPropertyAppName} found: Current ${this.apiVersion}, Maximum: ${kJSONPropertyMaximumSupportedAPIVersion}`);
                    else 
                        debug(`Wrong api version for ${kJSONPropertyAppName} found: Current ${this.apiVersion}, Minimum: ${kJSONPropertyMinimumSupportedAPIVersion}`);
                    this.emitEvent(kPropertyUpdatePI);
                    this.emitEvent(kPropertyOutputChanged);
                    this.emitEvent(kJSONPropertyInputsChanged);
                }
